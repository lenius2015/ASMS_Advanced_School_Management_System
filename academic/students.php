...
// Fetch registration completeness for all displayed students
$studentIds = array_column($students, 'student_id');
$regStatuses = [];
if (!empty($studentIds)) {
    $placeholders = implode(',', array_fill(0, count($studentIds), '?'));
    $regStmt = $pdo->prepare(
        "SELECT student_id, registration_complete FROM students WHERE student_id IN ($placeholders)"
    );
    $regStmt->execute($studentIds);
    foreach ($regStmt->fetchAll() as $r) {
        $regStatuses[(int) $r['student_id']] = (int) $r['registration_complete'];
    }
}
...
<div class="card">
  <div class="table-responsive">
    <table class="table table-hover mb-0">
      <thead><tr><th>Admission No.</th><th>Name</th><th>Class</th><th>Gender</th><th>Status</th><th>Reg.</th><th>Admitted</th><th></th></tr></thead>
      <tbody>
        <?php foreach ($students as $s): ?>
          <?php
            $sid = (int) $s['student_id'];
            $isComplete = isset($regStatuses[$sid]) && $regStatuses[$sid] === 1;
          ?>
          <tr>
            ...
            <td>
              ...
              <?php else: ?>
                <span class="badge bg-danger" title="Registration incomplete"><i class="fa fa-exclamation-circle"></i></span>
              <?php endif; ?>
            </td>
            <td class="small text-muted"><?= format_date($s['admission_date']) ?></td>
            <td><a href="<?= e(app_url('/director/student_profile.php')) ?>?id=<?= $sid ?>" class="btn btn-sm btn-outline-primary">View</a></td>
          </tr>
        <?php endforeach; ?>
