<?php
/**
 * class_teacher/discipline.php
 * Discipline records: log new incidents, track action taken, and
 * optionally notify parents/guardians. Class teachers see/log only for
 * their own homeroom class; head_of_school/department_head see all.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'log_incident') {
    ...
    if ($studentId <= 0 || $description === '') {
        ...
        redirect(app_url('/class_teacher/discipline.php'));
    }
}
...
if (current_role() === 'class_teacher') {
    $sql .= " JOIN classes c ON c.class_id = s.class_id WHERE c.class_teacher_id = :tid";
} else {
    $sql .= " WHERE 1=1";
}
$params = current_role() === 'class_teacher' ? ['tid' => current_user_id()] : [];
if ($search !== '') {
...
$students = current_role() === 'class_teacher'
    ? $pdo->prepare(
        "SELECT s.student_id, u.first_name, u.last_name, s.admission_no FROM students s
         JOIN users u ON u.user_id = s.user_id
         JOIN classes c ON c.class_id = s.class_id
         WHERE s.status='active' AND c.class_teacher_id = :tid ORDER BY u.first_name"
      )
    : $pdo->prepare(
        "SELECT s.student_id, u.first_name, u.last_name, s.admission_no FROM students s
         JOIN users u ON u.user_id = s.user_id WHERE s.status='active' ORDER BY u.first_name"
      );
$students->execute(current_role() === 'class_teacher' ? ['tid' => current_user_id()] : []);
